...
class ApisControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create api" do
    assert_difference('Api.count') do
      post apis_url, params: { api: { action: @api.action, response: @api.response, status: @api.status, uri: @api.uri } }
    end
  ...
  test "should update api" do
    patch api_url(@api), params: { api: { action: @api.action, response: @api.response, status: @api.status, uri: @api.uri } }
